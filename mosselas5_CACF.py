        ...
        for idx in range(len(self.frame_lists)):
            if idx == 0:
                ...
                if pred1[0][1] < 0.4:
                    ...
                    starting_point_y = int(np.floor(np.subtract(previous_position[0],window_sz[0])))
                    starting_point_x = int(np.floor(np.subtract(previous_position[1],window_sz[1])))
                    ending_point_y = int(np.floor(np.add(previous_position[0],window_sz[0])))
                    ending_point_x = int(np.floor(np.add(previous_position[1],window_sz[1])))
